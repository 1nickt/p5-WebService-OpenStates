...
use Types::Common::Numeric 'NumRange';
use JSON::MaybeXS;
use URI;
use Moo;
use namespace::clean;
...
has api_key => (
    ...
    isa      => NonEmptyStr,
    required => 1,
    default  => sub { $ENV{OPENSTATES_API_KEY} },
);
...
has _api_url => (
    is       => 'lazy',
    init_arg => undef,
    builder  => sub { 'https://v3.openstates.org/people.geo' },
);
...
##
method legislators_for_location ((NumRange[-90,90]) :$lat, (NumRange[-180,180]) :$lon) {
    ...
    my $response = try {
        if ( ! $call->{success} ) {
            my $resp = { status => 'error', content => decode_json( $call->{content} ) };
            return $resp;
        }
        else {
            ...
            for my $legislator (sort { $a->{family_name} cmp $b->{family_name} } @legislators) {
                my $result = {
                    ...
                    links   => [map {$_->{url}} @{$legislator->{links}}],
                    offices => [ map {+{name => $_->{name}, phone => $_->{voice}, address => $_->{address} }} @{$legislator->{offices}}],
                };

                push(@result, $result);
            }

            return { status => 'success', legislators => \@result };
        }
    }
    catch {
        return { status => 'error', content => "Caught fatal error trying to call Open States API: $_" };
    };
...
L<https://open.pluralpolicy.com/accounts/login/?next=/accounts/profile/#apikey>.
...
=item B<legislators_for_location (NumRange[-90,90] :$lat, NumRange[-180,180] :$lon)>

Requires a latitude numeric value as the first argument and a longtitude numeric
value as the second argument.

Returns a JSON object containing the C<status> key with values 'success'
